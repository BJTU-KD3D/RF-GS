...
class GaussianModel:
    ...
    def prune_points_by_opacity(self, min_opacity, extent=None, max_screen_size=None):
        """Remove Gaussians whose activated opacity is below ``min_opacity``.

        ``extent`` and ``max_screen_size`` are accepted for compatibility with
        the RF-GS training call; opacity is the sole pruning criterion here.
        """
        del extent, max_screen_size
